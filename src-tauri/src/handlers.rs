// this file will have all the core logic which we reciven
// for our frontend

use crate::db;
use crate::epub_util;
use crate::errors::ApplicationError;
use crate::models;
use crate::models::ContentResponse;
use crate::util;
use log::{error, info};
use sqlx::SqlitePool;
use tauri::{Manager, State};
use uuid::Uuid;

// this will be the first screen that user sees when he opens
// vaga read
#[tauri::command]
pub async fn show_home_page_handler(
    app: tauri::AppHandle,
    pool: State<'_, SqlitePool>,
) -> Result<Vec<models::Vagaread>, ApplicationError> {
    if let Ok(path) = app.path().app_data_dir() {
        info!("app data dir: {:?}", path);
    }

    list_all_books(pool.inner()).await.map_err(|e| {
        error!("failed to list books: {:?}", e);
        e
    })
}

// this handler is called when user uploads a file
#[tauri::command]
pub async fn upload_file_handler(
    app: tauri::AppHandle,
    req: models::UploadFileRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<ContentResponse, ApplicationError> {
    let req = models::UploadFileRequest::validate(req)?;
    load_file_core(&app, req.file_path(), pool.inner())
        .await
        .map_err(|e| {
            error!(
                "upload_file: failed to import '{}': {:?}",
                req.file_path(),
                e
            );
            e
        })
}

#[tauri::command]
pub async fn get_ebook_content_handler(
    _app: tauri::AppHandle,
    req: models::GetEbookContentRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<models::BookResponse, ApplicationError> {
    let req = models::GetEbookContentRequest::validate(req)?;
    get_ebook_content_paginated(
        pool.inner(),
        req.file_id(),
        req.spine_idx(),
        req.char_offset(),
    )
    .await
    .map_err(|e| {
        error!(
            "get_ebook_content: book={} spine={} offset={}: {:?}",
            req.file_id(),
            req.spine_idx(),
            req.char_offset(),
            e
        );
        e
    })
}

#[tauri::command]
pub async fn list_spine_handler(
    req: models::ListSpineRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<Vec<models::SpineItemResponse>, ApplicationError> {
    let req = models::ListSpineRequest::validate(req)?;
    let record = db::get_vb_record_by_id(pool.inner(), req.file_id().to_string())
        .await
        .map_err(|e| {
            error!("list_spine: db lookup failed id={}: {:?}", req.file_id(), e);
            e
        })?;
    epub_util::get_epub_spine(&record.internal_fp).map_err(|e| {
        error!(
            "list_spine: failed to parse spine book={}: {:?}",
            req.file_id(),
            e
        );
        e
    })
}

/// Returns the cover image as a data URI ("data:image/jpeg;base64,..."), or null if none.
#[tauri::command]
pub async fn get_cover_image_handler(
    req: models::GetCoverImageRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<Option<String>, ApplicationError> {
    let req = models::GetCoverImageRequest::validate(req)?;
    let record = db::get_vb_record_by_id(pool.inner(), req.file_id().to_string())
        .await
        .map_err(|e| {
            error!(
                "get_cover_image: db lookup failed id={}: {:?}",
                req.file_id(),
                e
            );
            e
        })?;
    epub_util::extract_cover_as_data_uri(&record.internal_fp).map_err(|e| {
        error!(
            "get_cover_image: failed to extract cover book={}: {:?}",
            req.file_id(),
            e
        );
        e
    })
}

/// Saves reading progress for within-chunk page turns (spine_idx + char_offset + visual page).
/// Called by the frontend whenever the user turns a page within the same content chunk.
#[tauri::command]
pub async fn save_reading_progress_handler(
    req: models::SaveReadingProgressRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<(), ApplicationError> {
    let req = models::SaveReadingProgressRequest::validate(req)?;
    let record = db::get_vb_record_by_id(pool.inner(), req.file_id().to_string())
        .await
        .map_err(|e| {
            error!(
                "save_reading_progress: db lookup failed id={}: {:?}",
                req.file_id(),
                e
            );
            e
        })?;
    db::update_vb_record(
        pool.inner(),
        models::UpdateVr {
            vagaread_id: record.vagaread_id.to_string(),
            current_read_idx: req.char_offset(),
            current_spine: req.spine_idx(),
            current_page: req.current_page(),
        },
    )
    .await
    .map_err(|e| {
        error!(
            "save_reading_progress: db update failed book={} spine={} offset={} page={}: {:?}",
            req.file_id(),
            req.spine_idx(),
            req.char_offset(),
            req.current_page(),
            e
        );
        e
    })
}

/// Saves the current speed-reader position. Called on pause, stop (word=0), back, and close.
/// Also updates the reading progress (spine/char/page) in the same call.
#[tauri::command]
pub async fn save_sr_position_handler(
    req: models::SaveSrPositionRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<(), ApplicationError> {
    let req = models::SaveSrPositionRequest::validate(req)?;
    let id = req.file_id().to_string();
    let pointer = format!("{}:{}", req.word_idx(), req.mode());
    // Save reading position and SR pointer atomically
    db::update_vb_record(
        pool.inner(),
        models::UpdateVr {
            vagaread_id: id.clone(),
            current_read_idx: req.char_offset(),
            current_spine: req.spine_idx(),
            current_page: req.current_page(),
        },
    )
    .await
    .map_err(|e| {
        error!(
            "save_sr_position: progress update failed book={}: {:?}",
            id, e
        );
        e
    })?;
    db::update_sr_position(pool.inner(), &id, &pointer)
        .await
        .map_err(|e| {
            error!(
                "save_sr_position: sr pointer update failed book={} pointer='{}': {:?}",
                id, pointer, e
            );
            e
        })
}

/// Returns the persisted global reader settings, or defaults if none saved yet.
#[tauri::command]
pub async fn get_settings_handler(
    pool: State<'_, SqlitePool>,
) -> Result<models::AppSettings, ApplicationError> {
    db::get_settings(pool.inner()).await.map_err(|e| {
        error!("get_settings: failed to load settings: {:?}", e);
        e
    })
}

/// Persists global reader settings (WPM, font sizes, colors, focus mode).
#[tauri::command]
pub async fn save_settings_handler(
    req: models::SaveSettingsRequestRaw,
    pool: State<'_, SqlitePool>,
) -> Result<(), ApplicationError> {
    let req = models::SaveSettingsRequest::validate(req)?;
    db::update_settings(pool.inner(), &req).await.map_err(|e| {
        error!("save_settings: failed to persist settings: {:?}", e);
        e
    })
}

// core logic shared by both handlers — takes &SqlitePool directly, no State wrapper
async fn list_all_books(pool: &SqlitePool) -> Result<Vec<models::Vagaread>, ApplicationError> {
    let records = db::list_all_vb_records(pool).await?;
    Ok(records)
}

// core logic shared by both handlers — takes &SqlitePool directly, no State wrapper
async fn load_file_core(
    app: &tauri::AppHandle,
    file_path: &str,
    pool: &SqlitePool,
) -> Result<ContentResponse, ApplicationError> {
    let new_fp = util::copy_to_app_directory(app, file_path).map_err(|e| {
        error!("load_file_core: copy failed for '{}': {:?}", file_path, e);
        e
    })?;

    let metadata_str = epub_util::extract_epub_metadata(&new_fp).map_err(|e| {
        error!(
            "load_file_core: metadata extraction failed '{}': {:?}",
            new_fp, e
        );
        e
    })?;

    let metadata: serde_json::Value = serde_json::from_str(&metadata_str).map_err(|e| {
        error!("load_file_core: metadata json parse failed: {e}");
        ApplicationError {
            code: crate::errors::codes::DATABASE_ERROR,
            message: Some(format!("invalid metadata json: {e}")),
        }
    })?;

    let vr_record = models::Vagaread {
        vagaread_id: Uuid::new_v4(),
        internal_fp: new_fp.to_string(),
        meta_data: metadata,
        current_read_idx: 0,
        current_spine: 0,
        current_page: 0,
        sr_word_idx: 0,
        sr_mode: "inline".to_string(), //TODO: this line is a bug
        is_deleted: false,
    };
    db::create_record(pool, vr_record).await.map_err(|e| {
        error!("load_file_core: db insert failed '{}': {:?}", new_fp, e);
        e
    })?;

    epub_util::get_paginated_content(&new_fp, 0, 0, models::PAGINATE_CHAR).map_err(|e| {
        error!(
            "load_file_core: failed to read first chunk '{}': {:?}",
            new_fp, e
        );
        e
    })
}

async fn get_ebook_content_paginated(
    pool: &SqlitePool,
    file_id: uuid::Uuid,
    spine_idx: usize,
    char_offset: usize,
) -> Result<models::BookResponse, ApplicationError> {
    let record = db::get_vb_record_by_id(pool, file_id.to_string())
        .await
        .map_err(|e| {
            error!(
                "get_ebook_content_paginated: db lookup failed id={}: {:?}",
                file_id, e
            );
            e
        })?;

    let restore_page;
    if spine_idx == record.current_spine && char_offset == record.current_read_idx {
        // Requested position matches what's saved in DB — this is either opening the book
        // or returning to the same chunk. Return the saved visual page so the frontend
        // can restore the exact page the user was on. Do NOT save (nothing changed).
        restore_page = record.current_page;
    } else {
        // Navigating to a new chunk — save new position; new chunk starts at visual page 0
        restore_page = 0;
        update_ebook_page_state_async(
            pool.clone(),
            models::UpdateVr {
                vagaread_id: record.vagaread_id.to_string(),
                current_read_idx: char_offset,
                current_spine: spine_idx,
                current_page: 0,
            },
        ); // fire and forget — does not block page load
    }

    let mut content = epub_util::get_paginated_content(
        &record.internal_fp,
        spine_idx,
        char_offset,
        models::PAGINATE_CHAR,
    )
    .map_err(|e| {
        error!(
            "get_ebook_content_paginated: content read failed book={} spine={} offset={}: {:?}",
            file_id, spine_idx, char_offset, e
        );
        e
    })?;
    content.current_page = restore_page;
    Ok(models::BookResponse {
        vagaread_id: record.vagaread_id,
        content,
    })
}

fn update_ebook_page_state_async(pool: SqlitePool, data_model: models::UpdateVr) {
    tauri::async_runtime::spawn(async move {
        if let Err(e) = db::update_vb_record(&pool, data_model).await {
            error!("failed to save page state: {:?}", e);
        }
    });
}
